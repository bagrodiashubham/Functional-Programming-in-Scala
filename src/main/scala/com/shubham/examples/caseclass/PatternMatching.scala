...
object PatternMatching {
  def matchElement(x:Any): Unit = {
    val matchedElement  = x match {
      // matches a List(1,2,3)
      case List(1, 2, 3) => "Matched List(1, 2, 3)"

      // Matches a list with only one element and extracts the element by decomposing the list
      ...
      case _ => "Empty List"
      // Matching a empty list using constructor decomposition??
