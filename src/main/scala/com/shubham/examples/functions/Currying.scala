package com.shubham.examples.functions
...
object Currying {
  ...
  //Normal function
  def communicate(t:Text,s:Source): Unit = s.sendText(t)

  //Scala currying way
  def communicateFrom(t:Text): Source => Unit = (s:Source) => s.sendText(t)

  //or
  def communicateFrom1(t:Text)(s:Source): Unit = s.sendText(t)
